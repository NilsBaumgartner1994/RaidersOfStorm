...
export class DateHelper {
	...
	static getDatesOfAmountNextDaysIncludingToday(startDate: Date, amount: number): [Date]{
		let dates: [Date] = [];
	...
	static getSmartReadableDate(date: Date, locale?: string){
		let today = new Date();
		...
		if(DateHelper.isSameDay(today, date)){
			let todayTranslation = new Intl.RelativeTimeFormat(locale, { numeric: 'auto' }).format(0, 'day');
			// capitalize first letter
			todayTranslation = todayTranslation.charAt(0).toUpperCase() + todayTranslation.slice(1);
			return todayTranslation;
		}
		// check if date is tomorrow, then return "tomorrow"
		if(DateHelper.isSameDay(tomorrow, date)){
			let tomorrowTranslation = new Intl.RelativeTimeFormat(locale, { numeric: 'auto' }).format(1, 'day');
			// capitalize first letter
			tomorrowTranslation = tomorrowTranslation.charAt(0).toUpperCase() + tomorrowTranslation.slice(1);
			return tomorrowTranslation;
		}
		// check if date is yesterday, then return "yesterday"

		if(DateHelper.isSameDay(yesterday, date)){
			let yesterdayTranslation = new Intl.RelativeTimeFormat(locale, { numeric: 'auto' }).format(-1, 'day');
			// capitalize first letter
			yesterdayTranslation = yesterdayTranslation.charAt(0).toUpperCase() + yesterdayTranslation.slice(1);
