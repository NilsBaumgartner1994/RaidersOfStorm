...
export const SettingsRowTextEditComponent: FunctionComponent<AppState> = (props) => {
        ...
        return(
            <View style={{width: "100%"}}>
                ...
                <ParentSpacer space={10} style={{marginTop: 20, width: "100%", flex: 1, flexDirection: "row"}}>
                    <View style={{flex: 1}}>
                        <MyButton icon={<SaveIcon />} accessibilityLabel={props?.saveText} disabled={!saveEnabled} onPress={async () => {
                            if(!!props.onChange){
                                setLoading(true);
                            } else {
                                //TODO: error handling
                    ...
                    <View style={{flex: 1}}>
                        <MyButton icon={<Icon name={"cancel"} />} accessibilityLabel={props?.cancelText} disabled={loading} onPress={() => {
                            setEditText(false);
  ...
  return (
      <SettingsRow accessibilityLabel={translationEdit+": "+description} expanded={editText} onPress={editText ? undefined : () => {if(!editText) {setEditText(!editText)}}} leftContent={description} rightContent={renderContent()} leftIcon={props?.icon} >
          {renderEditActions()}
      </SettingsRow>
  )
}
